...
def convolve(image, kernel):
  ...
  rows=image.shape[0]
  cols=image.shape[1]
  output=np.ndarray(shape=(rows-d+1,cols-d+1),dtype=np.uint8)
  
  kernelFlip=flipKernel(kernel)
  for row in xrange(d/2,rows-d/2):
    for col in xrange(d/2,cols-d/2):
      sumVal=0
      for kerX in xrange(d):
        for kerY in xrange(d):
          sumVal+=kernelFlip[kerX,kerY]*image[row-d/2+kerX,col-d/2+kerY]
          if sumVal>255: 
            sumVal=255
          if sumVal<0:
            sumVal=0
      output[row-d/2,col-d/2]=sumVal
