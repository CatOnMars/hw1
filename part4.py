...
def transform_xy_theta(dx, dy):
  ...
  rows=dx.shape[0]
  cols=dx.shape[1]
 # theta=np.ndarray(shape=(rows,cols),dtype=float)
 # for i in xrange(rows):
 #   for j in xrange(cols):
 #     theta[i,j]=math.arctan(dy[i,j]/dx[i,j])
...
def transform_xy_mag(dx, dy):
  ...
  rows=dx.shape[0]
  cols=dx.shape[1]
 # mag=np.ndarray(shape=(rows,cols),type=float)
 # for i in xrange(rows):
 #   for j in xrange(cols):
 #     mag[i,j]=math.sqrt(math.square(dy[i,j])+math.square(dx[i,j])
