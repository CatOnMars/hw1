...
def make_gaussian(k, std):
  ...
  kernel =np.ndarray(shape=(2*k+1,2*k+1),dtype=float)
  for row in xrange(kernel.shape[0]):
    for col in xrange(kernel.shape[1]):
      kernel[row,col]=gaussian_1d[row]*gaussian_1d[col]
