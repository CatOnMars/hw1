...
def filter_median(image, k):
  ...
  rows=image.shape[0]
  cols=image.shape[1]
  output=np.ndarray(shape=(rows-2*k,cols-2*k),dtype=image.dtype)
  for row in xrange(k,rows-k):
    for col in xrange(k,cols-k):
      patch=image[row-k:row+k+1,col-k:col+k+1]
      output[row-k,col-k]=np.median(patch)
  #---------------------------------------------------------------------------
  return output 
